...
show_help() {
    ...
    echo "  -t, --target         指定构建目标 (x86_64-apple-darwin|aarch64-apple-darwin|all)"
    echo "  -o, --output         指定输出目录 (默认: ./release)"
    echo ""
    echo "示例:"
    echo "  $0                              # 构建所有目标"
    echo "  $0 -t aarch64-apple-darwin      # 仅构建 Apple Silicon 版本"
...
check_build_environment() {
    ...
    if ! command_exists xcode-select; then
        errors+=("Xcode 命令行工具未安装。运行: xcode-select --install")
    else
        log "Xcode 命令行工具已安装" "success"
    fi
...
build_tauri() {
    ...
    log "Tauri 构建完成 (目标: $target)" "success"
}
...
copy_build_artifacts() {
    ...
    # 复制 DMG
    local dmg_files=$(find "./src-tauri/target/$target/release/bundle/dmg" -name "*.dmg" 2>/dev/null || true)
    for file in $dmg_files; do
        local dest_name="Nexus_${version}_${arch}.dmg"
        cp "$file" "$OUTPUT_PATH/$dest_name"
        log "已复制: $dest_name"
    done
    
    # 复制 APP (打包为 zip)
    local app_files=$(find "./src-tauri/target/$target/release/bundle/macos" -name "*.app" 2>/dev/null || true)
    for app in $app_files; do
        local app_name=$(basename "$app")
        local dest_name="Nexus_${version}_${arch}.app.tar.gz"
        tar -czf "$OUTPUT_PATH/$dest_name" -C "$(dirname "$app")" "$app_name"
        log "已复制: $dest_name"
    done
...
main() {
    echo ""
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  Nexus macOS 构建脚本${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo ""
